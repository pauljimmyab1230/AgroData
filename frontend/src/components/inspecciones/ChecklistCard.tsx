...
function contarCumplimiento(criterios: CriterioChecklist[]) {
  return {
    ...
    noCumple: criterios.filter((c) => c.cumplimiento === "No Cumple").length,
    noAplica: criterios.filter((c) => c.cumplimiento === "No Aplica").length,
...
export function ChecklistCard({ mode, values }: ChecklistCardProps) {
  ...
  const setCumplimiento = (id: number, cumplimiento: Cumplimiento) => {
    setCriterios((prev) => prev.map((c) => (c.id === id ? { ...c, cumplimiento } : c)));
  };

  const setCampo = (id: number, campo: "riesgo" | "observacion" | "evidencia", valor: string) => {
    setCriterios((prev) =>
      prev.map((c) => (c.id === id ? ({ ...c, [campo]: valor } as CriterioChecklist) : c)),
    );
  };
    ...
    <CardShell>
      ...
      <div className="space-y-3">
        {list.map((criterio, index) => (
          <div key={criterio.id} className="rounded-xl border border-gray-200 bg-white p-4">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  {opciones.map((op) => (
                    <button
                      key={op.value}
                      type="button"
                      onClick={() => setCumplimiento(criterio.id, op.value)}
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Field label="Observación" mode={mode} value={criterio.observacion} className="sm:col-span-2 lg:col-span-1">
                  <Input
                    ...
                    onChange={(e) => setCampo(criterio.id, "observacion", e.target.value)}
                  />
                </Field>
                <Field label="Evidencia asociada" mode={mode} value={criterio.evidencia} className="sm:col-span-2 lg:col-span-1">
                  <Input
                    ...
                    onChange={(e) => setCampo(criterio.id, "evidencia", e.target.value)}
                  />
                </Field>
                <Field label="Nivel de riesgo" mode={mode} value={criterio.riesgo}>
                  <Select
                    ...
                    onChange={(val) => setCampo(criterio.id, "riesgo", val as Riesgo)}
                  />
                </Field>
              </div>
            ) : (
