...
import { formatFecha, formatKg, type Recepcion } from "../../pages/recepcion/recepcionMock";
...
export default function RecepcionTable({
  ...
  const columns = [
    { key: "codigo", label: "Código", sortable: true, className: "font-medium text-forest-700" },
    {
      ...
      label: "Fecha",
      sortable: true,
      render: (recepcion: Recepcion) => <span className="text-gray-600">{formatFecha(recepcion.fecha)}</span>,
    },
    {
      key: "loteProductor",
      label: "LP",
    ...
    },
    {
      ...
      render: (recepcion: Recepcion) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-[#111827]">{recepcion.productor}</p>
            <p className="text-xs text-gray-500">{recepcion.cultivo}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      sortable: true,
      render: (recepcion: Recepcion) => (
        <span className="font-medium text-[#111827]">{formatKg(recepcion.pesoCampo)}</span>
      ),
    },
    {
      key: "pesoNeto",
      label: "Peso Planta",
      sortable: true,
      render: (recepcion: Recepcion) => <span className="font-medium text-[#111827]">{formatKg(recepcion.pesoNeto)}</span>,
    },
    {
      key: "estado",
      label: "Estado",
