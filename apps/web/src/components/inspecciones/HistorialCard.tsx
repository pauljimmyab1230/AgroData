import { CalendarClock, Camera, ClipboardCheck, Flag, History, MessageSquare, ShieldCheck, Wrench, type LucideIcon } from "lucide-react";
import { CardHeader, CardShell } from "../shared/formControls";
import { formatFecha, type EventoHistorial, type TipoEvento } from "../../services/inspecciones";

type HistorialCardProps = {
  eventos: EventoHistorial[];
};

const config: Record<TipoEvento, { icon: LucideIcon; circle: string }> = {
  programada: { icon: CalendarClock, circle: "bg-amber-100 text-amber-700" },
  realizada: { icon: ClipboardCheck, circle: "bg-forest-600/10 text-forest-600" },
  evidencias: { icon: Camera, circle: "bg-purple-100 text-purple-700" },
  observaciones: { icon: MessageSquare, circle: "bg-sky-100 text-sky-700" },
  correctivas: { icon: Wrench, circle: "bg-orange-100 text-orange-700" },
  verificacion: { icon: ShieldCheck, circle: "bg-emerald-100 text-emerald-700" },
  cierre: { icon: Flag, circle: "bg-gray-100 text-gray-600" },
};
      ...
      <div className="relative">
        ...
        <ol className="space-y-6">
          {eventos.map((evento, index) => {
            const conf = config[evento.tipo] ?? config.programada;
