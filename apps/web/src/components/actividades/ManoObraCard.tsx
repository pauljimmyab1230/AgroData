...
import {
  ...
  type ActividadManoObra as Trabajador,
} from "../../services/actividades";

const trabajadoresOpciones = [
  "Santos Huamán",
  "Félix Condori",
  "Marcelina Rojas",
  "Gregorio Quispe",
...
export function ManoObraCard({ mode, value, onChange }: ManoObraCardProps) {
  ...
  const openEdit = (trabajador: Trabajador) => {
    setEditingId(trabajador.id ?? null);
    setDraft({ ...trabajador });
    setOpen(true);
  };
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <FormField label="Jornales" required>
            <Input
              ...
              onChange={(e) => setDraft((d) => ({ ...d, jornales: e.target.value ? Number(e.target.value) : null }))}
            />
          </FormField>
