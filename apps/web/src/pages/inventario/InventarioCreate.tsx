...
export default function InventarioCreate() {
  ...
  const [form, setForm] = useState<InventarioItemFormData>({ ...emptyInventarioItemForm });
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const created = await createInventarioItem(form);
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: "Inventario", to: "/inventario" }, { label: "Nuevo Item" }]} />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          <Button variant="ghost" type="button" as="link" to="/inventario">
            Cancelar
          </Button>
