...
import { tiposActividad, tipoActividadLabels, type ActividadFormData } from "../../services/actividades";
...
const proximaOptions = tiposActividad.map((t) => ({ value: t.value, label: t.label }));
  ...
  return (
    <CardShell>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <Field label="Próxima Actividad Recomendada" mode={mode} value={tipoActividadLabels[value.proximaActividad] ?? value.proximaActividad}>
            {editable ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 rounded-xl border border-forest-100 bg-forest-50/60 px-4 py-3">
                  ...
                  <p className="text-sm font-medium text-forest-700">{(tipoActividadLabels[value.proximaActividad] ?? value.proximaActividad) || "—"}</p>
                </div>
              </div>
            )}
          </Field>
