...
export const getAll = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const parcelas = await parcelasService.getAll({
...
export const create = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const parcela = await parcelasService.create(req.body);
    res.status(201).json({
      success: true,
...
export const update = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const parcela = await parcelasService.update(req.params.id, req.body);
    res.status(200).json({
      success: true,
