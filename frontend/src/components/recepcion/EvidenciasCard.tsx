...
import type { EvidenciaRecepcion, Recepcion } from "../../pages/recepcion/recepcionMock";
...
export function EvidenciasCard({ mode, values }: EvidenciasCardProps) {
  const editable = mode !== "view";
  const [fotos, setFotos] = useState<EvidenciaRecepcion[]>(values?.evidencias ?? []);
  const [documentoRecepcion, setDocumentoRecepcion] = useState(values?.documentoRecepcion ?? false);
  const [firmaResponsable, setFirmaResponsable] = useState(values?.firmaResponsable ?? false);
  ...
  const addFoto = () => {
    ...
    setFotos((prev) => [
      ...prev,
      { id: n, nombre: `Fotografía ${n} - Recepción`, descripcion: "Fotografía registrada durante la recepción" },
    ]);
  };
    ...
    <CardShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <RegistroTile
          ...
          registrado={documentoRecepcion}
          tipo="documento"
          onToggle={() => setDocumentoRecepcion((prev) => !prev)}
        />
        <RegistroTile
          mode={mode}
          ...
          registrado={firmaResponsable}
          tipo="firma"
          onToggle={() => setFirmaResponsable((prev) => !prev)}
        />
      </div>
