...
export function ActividadTimeline({ actividades, current }: ActividadTimelineProps) {
  const historial = actividades
    .filter((a) => a.parcelaNombre === current.parcelaNombre)
    .sort((a, b) => {
  ...
  return (
    <CardShell>
      <CardHeader
        ...
        description={`${current.parcelaNombre} · Ciclo del cultivo: ${current.cultivoNombre}`}
      />
              ...
              <li key={actividad.id} className="relative flex gap-4 pb-6 last:pb-0">
                ...
                <div className="min-w-0 flex-1 pb-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-[#111827]">{tipoActividadLabels[actividad.tipoActividad] ?? actividad.tipoActividad}</span>
                    <ActividadEstadoBadge estado={actividad.estado} />
                    <ActividadPrioridadBadge prioridad={actividad.prioridad} />
                  </div>

                  <p className="mt-1 text-xs text-gray-500">
                    {formatearFecha(actividad.fecha)} · {actividad.productorNombre} · {actividad.responsableTecnico}
                  </p>
                </div>
              </li>
            );
