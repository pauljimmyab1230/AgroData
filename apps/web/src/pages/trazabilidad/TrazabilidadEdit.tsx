...
import { Breadcrumb, Button, Card, FormField, Input, Select, Textarea, LoadingSpinner, SectionHeader } from "../../components/ui";
import {
  fetchTrazabilidad,
  updateTrazabilidad,
  trazabilidadEstados,
...
function toForm(data: Trazabilidad): TrazabilidadFormData {
  return {
    codigo: data.codigo,
    loteId: data.loteId,
...
export default function TrazabilidadEdit() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [form, setForm] = useState<TrazabilidadFormData | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await updateTrazabilidad(id, form);
      navigate(`/trazabilidad/${id}`);
    } catch {
      setSaving(false);
    }
  };
    ...
    <div>
      <Breadcrumb
        items={[
          ...
          { label: form.codigo, to: `/trazabilidad/${id}` },
          { label: "Editar Registro" },
        ]}
      />

      <div className="mb-8 flex items-center gap-4">
        <Button variant="ghost" as="link" to={`/trazabilidad/${id}`} iconLeft={<ArrowLeft className="h-4 w-4" />}>
          Volver
        </Button>
        <SectionHeader
          title="Editar Registro"
          description={`Actualizando la informacion de trazabilidad ${form.codigo}`}
        />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button variant="ghost" as="link" to={`/trazabilidad/${id}`}>
            Cancelar
          </Button>
